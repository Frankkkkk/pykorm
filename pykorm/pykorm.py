...
class CustomObjectApi(BaseQuery):
    group = None
    version = None
    plural = None

    def _create(self):
        print(f'Will create on {self.group}/{self.version}/{self.plural}')


class AbstractPykorm:
    __query_cls = BaseQuery
    @property
    def query(self) -> BaseQuery:
        return self.__query_cls(self)



# XXX is namespaced or cluster


#class MyDB(CustomObjectApiFactory('group1', 'v1', 'plurals')):
#    pass
...
#    # Code.Node
#
#    def __init__(self, kubeconfig='/etc/kfg'):
